...
const GunArt = {
  ...
  parts(key, sk) {
    ...
    const rail = (len, y, z) => [
      ...
      B.box(0.010, 0.020, len, D, -0.013, y + 0.010, z),
      B.box(0.010, 0.020, len, D, 0.013, y + 0.010, z)
    ];
    // 방아쇠울 + 손잡이 (기울여 쥐는 각도까지 같게)
    const grip = (x, y, z, h, tilt) => [
      B.box(0.048, h, 0.075, P, x, y, z, tilt || 0.28),
      B.box(0.052, 0.014, 0.085, D, x, y + h / 2 - 0.01, z + 0.012),
      B.box(0.040, 0.030, 0.010, D, x, y + h / 2 + 0.012, z + 0.055)
    ];

    switch (GUNS[key].model || key) {
      case 'pistol': return [
        B.box(0.048, 0.072, 0.235, M, 0, 0.030, 0.030),          // 슬라이드
        B.box(0.036, 0.020, 0.245, D, 0, 0.068, 0.028),          // 슬라이드 홈
        B.box(0.052, 0.060, 0.150, D, 0, -0.022, 0.000),         // 프레임
        B.tube(0.011, 0.05, D, 0, 0.030, 0.170),                 // 총구
        B.box(0.046, 0.115, 0.062, P, 0, -0.095, -0.048, 0.30),  // 손잡이
        B.box(0.030, 0.020, 0.055, D, 0, -0.052, 0.036),         // 방아쇠울
        B.box(0.014, 0.014, 0.014, D, 0, 0.072, 0.140),          // 가늠쇠
        B.box(0.030, 0.014, 0.014, D, 0, 0.072, -0.070),         // 가늠자
        B.box(0.030, 0.008, 0.095, A, 0, 0.058, 0.055)           // 탄 표시등
      ];

      case 'smg': return [].concat(
        [B.box(0.062, 0.100, 0.235, M, 0, 0.020, 0.030),         // 몸통
         B.box(0.070, 0.058, 0.140, M, 0, 0.008, 0.215),         // 총열 덮개
         B.tube(0.015, 0.10, D, 0, 0.020, 0.320),
         B.tube(0.024, 0.045, D, 0, 0.020, 0.385),               // 소염기
         B.box(0.040, 0.150, 0.062, D, 0, -0.090, 0.055, 0.06),  // 탄창
         B.box(0.030, 0.070, 0.050, D, -0.048, 0.010, -0.120, 0, 0, 0.35),
         B.box(0.030, 0.070, 0.050, D, 0.048, 0.010, -0.120, 0, 0, -0.35),
         B.box(0.026, 0.030, 0.150, D, 0, 0.005, -0.140),        // 접이식 개머리판
         B.box(0.052, 0.014, 0.170, A, 0, 0.055, 0.045)],
        grip(0, -0.080, -0.070, 0.115, 0.24), rail(0.22, 0.078, 0.05));

      case 'shotgun': return [].concat(
        [B.tube(0.024, 0.58, M, 0, 0.048, 0.310),                // 위 총열
         B.tube(0.024, 0.58, M, 0, 0.002, 0.310),                // 아래 총열
         B.box(0.062, 0.020, 0.560, D, 0, 0.025, 0.310),         // 총열 사이 리브
         B.box(0.078, 0.105, 0.185, M, 0, 0.018, -0.030),        // 약실
         B.box(0.070, 0.062, 0.210, P, 0, 0.000, 0.175),         // 총열 덮개
         B.box(0.062, 0.115, 0.290, P, 0, -0.048, -0.290, -0.10),// 개머리판
         B.box(0.066, 0.130, 0.028, D, 0, -0.072, -0.435),
         B.box(0.018, 0.020, 0.045, A, 0, 0.078, 0.590),         // 가늠쇠
         B.box(0.040, 0.010, 0.120, A, 0, 0.072, -0.030)],
        grip(0, -0.062, -0.090, 0.075, 0.22));

      case 'rifle': return [].concat(
        [B.box(0.068, 0.110, 0.360, M, 0, 0.020, 0.010),         // 몸통
         B.box(0.062, 0.078, 0.300, M, 0, 0.016, 0.330),         // 총열 덮개
         B.box(0.014, 0.052, 0.290, D, -0.034, 0.016, 0.330),    // 덮개 옆 방열 홈
         B.box(0.014, 0.052, 0.290, D, 0.034, 0.016, 0.330),
         B.tube(0.016, 0.24, D, 0, 0.016, 0.580),
         B.tube(0.026, 0.055, D, 0, 0.016, 0.715),               // 소염기
         B.box(0.046, 0.190, 0.072, D, 0, -0.115, 0.030, 0.05),  // 탄창
         B.box(0.030, 0.075, 0.055, A, 0, -0.185, 0.024, 0.05),  // 탄창 바닥 표시
         B.box(0.054, 0.095, 0.230, P, 0, 0.000, -0.290),        // 개머리판
         B.box(0.048, 0.030, 0.120, D, 0, 0.052, -0.250),
         B.box(0.040, 0.010, 0.300, A, 0, 0.078, 0.060)],        // 몸통 표시등
        grip(0, -0.105, -0.105, 0.135, 0.26), rail(0.34, 0.082, 0.14));

      case 'dmr': return [].concat(
        [B.box(0.064, 0.108, 0.330, M, 0, 0.020, 0.020),
         B.box(0.058, 0.070, 0.280, M, 0, 0.022, 0.320),
         B.tube(0.017, 0.36, D, 0, 0.022, 0.560),
         B.tube(0.025, 0.050, D, 0, 0.022, 0.760),
         B.box(0.046, 0.170, 0.070, D, 0, -0.105, 0.060, 0.05),
         B.box(0.060, 0.105, 0.290, P, 0, -0.020, -0.290, -0.06), // 개머리판
         B.box(0.056, 0.036, 0.130, D, 0, 0.048, -0.230),         // 볼받침
         B.box(0.038, 0.010, 0.260, A, 0, 0.078, 0.060)],
        grip(0, -0.095, -0.075, 0.125, 0.26), rail(0.30, 0.080, 0.13));

      case 'sniper': return [].concat(
        [B.box(0.072, 0.118, 0.400, M, 0, 0.020, 0.040),
         B.tube(0.019, 0.52, D, 0, 0.024, 0.520),
         B.tube(0.030, 0.070, D, 0, 0.024, 0.800),               // 총구 제동기
         B.box(0.066, 0.130, 0.380, P, 0, -0.028, -0.310, -0.05),// 개머리판
         B.box(0.060, 0.040, 0.150, D, 0, 0.052, -0.250),
         B.box(0.044, 0.120, 0.062, D, 0, -0.090, 0.070, 0.04),
         B.box(0.040, 0.010, 0.320, A, 0, 0.084, 0.070),
         B.box(0.018, 0.150, 0.018, D, 0.042, -0.070, 0.560, 0, 0, -0.32),  // 양각대
         B.box(0.018, 0.150, 0.018, D, -0.042, -0.070, 0.560, 0, 0, 0.32),
         B.box(0.090, 0.014, 0.020, D, 0, -0.140, 0.560)],
        grip(0, -0.100, -0.060, 0.130, 0.28), rail(0.36, 0.086, 0.16));

      case 'vss': return [].concat(
        [B.box(0.062, 0.104, 0.280, D, 0, 0.020, 0.000),
         B.tube(0.034, 0.42, D, 0, 0.022, 0.350),                // 총열을 감싼 소음기
         B.box(0.056, 0.012, 0.380, D, 0, 0.056, 0.350),         // 소음기 위 방열판
         B.tube(0.040, 0.045, M, 0, 0.022, 0.575),
         B.box(0.044, 0.150, 0.066, D, 0, -0.095, 0.030, 0.05),
         B.box(0.056, 0.100, 0.240, P, 0, -0.010, -0.250),       // 골조 개머리판
         B.box(0.048, 0.026, 0.110, D, 0, 0.044, -0.210),
         B.box(0.036, 0.010, 0.200, A, 0, 0.076, 0.020),
         // 붙박이 조준경
         B.tube(0.030, 0.20, D, 0, 0.160, 0.040),
         B.tube(0.038, 0.030, M, 0, 0.160, 0.150),
         B.box(0.026, 0.055, 0.026, D, 0, 0.115, -0.030),
         B.box(0.026, 0.055, 0.026, D, 0, 0.115, 0.120),
         B.box(0.020, 0.016, 0.014, 0x64e2d8, 0, 0.160, -0.062)],
        grip(0, -0.090, -0.080, 0.120, 0.26));

      case 'lmg': return [].concat(
        [B.box(0.078, 0.125, 0.420, M, 0, 0.020, 0.010),
         B.tube(0.019, 0.36, D, 0, 0.024, 0.430),
         B.box(0.052, 0.016, 0.300, D, 0, 0.058, 0.430),         // 총열 위 방열판
         B.tube(0.029, 0.065, D, 0, 0.024, 0.630),
         B.box(0.110, 0.135, 0.185, P, 0, -0.040, 0.020),        // 탄통
         B.box(0.092, 0.020, 0.150, A, 0, -0.108, 0.020),        // 탄통 걸쇠
         B.box(0.058, 0.100, 0.250, P, 0, 0.000, -0.330),        // 개머리판
         B.box(0.050, 0.030, 0.130, D, 0, 0.052, -0.290),
         B.box(0.040, 0.010, 0.340, A, 0, 0.090, 0.060),
         B.box(0.018, 0.165, 0.018, D, 0.046, -0.075, 0.480, 0, 0, -0.36),
         B.box(0.018, 0.165, 0.018, D, -0.046, -0.075, 0.480, 0, 0, 0.36)],
        grip(0, -0.110, -0.120, 0.140, 0.26), rail(0.30, 0.090, 0.10));
...
const LootArt = {
  ...
  scopeItemParts(level) {
    const B = Build, D = 0x24282e, M = 0x555d66;
    const tint = SCOPES[level].color;
    if (level <= 2) return [                                   // 레드도트
      B.tube(0.052, 0.135, D, 0, 0.150, 0),                    // 짧은 경통
      B.tube(0.062, 0.022, M, 0, 0.150, 0.072),                // 앞 테두리
      B.tube(0.044, 0.012, tint, 0, 0.150, 0.080),             // 렌즈
      B.tube(0.058, 0.022, M, 0, 0.150, -0.072),               // 뒤 테두리
      B.box(0.040, 0.042, 0.040, M, 0, 0.198, 0.010),          // 밝기 조절기
      B.box(0.055, 0.070, 0.055, D, 0, 0.078, 0),              // 마운트
      B.box(0.135, 0.030, 0.115, M, 0, 0.040, 0)               // 레일 받침
    ];
    const big = level >= 8;
    const len = big ? 0.42 : 0.32, r = big ? 0.058 : 0.048;
    return [
      B.tube(r, len, D, 0, 0.150, 0),                          // 경통
      B.tube(r * 1.35, 0.075, M, 0, 0.150, len / 2 + 0.02),    // 대물 렌즈통
      B.tube(r * 1.28, 0.014, tint, 0, 0.150, len / 2 + 0.058),// 렌즈
      B.tube(r * 1.12, 0.050, M, 0, 0.150, -len / 2 - 0.02),   // 접안부
      B.box(0.045, 0.050, 0.045, M, 0, 0.205, 0.020),          // 위 조절 손잡이
      B.box(0.050, 0.045, 0.045, M, 0.052, 0.150, 0.020),      // 옆 조절 손잡이
      B.box(0.052, 0.075, 0.050, D, 0, 0.078, len * 0.22),     // 마운트 링
      B.box(0.052, 0.075, 0.050, D, 0, 0.078, -len * 0.22),
      B.box(0.150, 0.030, 0.130, M, 0, 0.040, 0)               // 레일 받침
    ];
  },
  ...
  /* 바닥에 떨어진 헬멧 */
  helmetParts(level) {
    const B = Build, c = HELMETS[level].color, D = 0x24282e, M = 0x555d66;
    const glow = level >= 3 ? 0x64e2d8 : 0x8fa3b5;
    const parts = [
      B.sphere(0.180, c, 0, 0.150, -0.020, 1.0, 0.62, 1.02, 16),   // 낮게 눌린 껍데기
      B.box(0.235, 0.050, 0.115, c, 0, 0.150, 0.150),              // 앞으로 뻗은 챙
      B.box(0.290, 0.045, 0.120, c, 0, 0.135, -0.140),             // 뒤 목가리개
      B.box(0.060, 0.115, 0.105, D, -0.155, 0.118, -0.010),        // 귀덮개
      B.box(0.060, 0.115, 0.105, D, 0.155, 0.118, -0.010),
      B.box(0.195, 0.055, 0.045, glow, 0, 0.110, 0.180),           // 앞 바이저 (챙 아래로 보입니다)
      B.box(0.205, 0.022, 0.050, D, 0, 0.142, 0.182),              // 바이저 위 테두리
      B.box(0.32, 0.045, 0.32, D, 0, 0.048, 0)                     // 받침대
    ];
    if (level >= 2) {                                              // 옆 레일
      parts.push(B.box(0.022, 0.030, 0.140, M, -0.170, 0.180, 0.000));
      parts.push(B.box(0.022, 0.030, 0.140, M, 0.170, 0.180, 0.000));
    }
    if (level >= 3) {                                              // 윗면 레일 + 야시경 거치대
      parts.push(B.box(0.150, 0.030, 0.036, M, 0, 0.252, -0.020));
      parts.push(B.box(0.062, 0.062, 0.062, D, 0, 0.205, 0.165));
    }
    return parts;
  },
  ...
  /* 바닥에 떨어진 방탄조끼 */
  vestParts(level) {
    const B = Build, c = VESTS[level].color, D = 0x24282e, M = 0x555d66;
    const glow = level >= 3 ? 0x64e2d8 : 0x8fa3b5;
    const parts = [
      B.box(0.300, 0.360, 0.075, c, 0, 0.320, 0.045),        // 앞판
      B.box(0.300, 0.320, 0.055, D, 0, 0.320, -0.045),       // 뒤판
      B.box(0.085, 0.300, 0.150, D, -0.190, 0.320, 0),       // 옆구리 띠
      B.box(0.085, 0.300, 0.150, D, 0.190, 0.320, 0),
      B.box(0.075, 0.090, 0.155, c, -0.150, 0.470, 0),       // 어깨
      B.box(0.075, 0.090, 0.155, c, 0.150, 0.470, 0),
      B.box(0.230, 0.020, 0.085, D, 0, 0.400, 0.050),        // 앞판 이음매
      B.box(0.075, 0.095, 0.045, M, -0.085, 0.230, 0.075),   // 탄창 주머니
      B.box(0.075, 0.095, 0.045, M, 0.085, 0.230, 0.075),
      B.box(0.060, 0.030, 0.035, glow, 0, 0.455, 0.075)      // 가슴 표시등
    ];
    for (let i = 0; i < level; i++) {
      parts.push(B.box(0.030, 0.030, 0.020, glow, -0.045 + i * 0.045, 0.150, 0.080));
    }
    return parts;
  },
  ...
  /* 바닥에 떨어진 가방 */
  bagParts(level) {
    const B = Build, c = BAGS[level].color, D = 0x24282e, M = 0x555d66;
    const glow = level >= 3 ? 0x64e2d8 : 0x8fa3b5;
    const w = 0.26 + level * 0.022, h = 0.28 + level * 0.030, d = 0.15 + level * 0.018;
    const parts = [
      B.box(w, h, d, c, 0, h / 2 + 0.03, 0),                       // 몸통
      B.box(w * 0.94, h * 0.26, d * 1.06, D, 0, h * 0.88, 0),      // 윗덮개
      B.box(w * 0.62, h * 0.30, d * 0.55, c, 0, h * 0.36, d * 0.72), // 앞주머니
      B.box(w * 0.66, 0.022, 0.030, M, 0, h * 0.50, d * 0.99),     // 앞주머니 지퍼
      B.box(0.055, h * 0.92, 0.045, D, -w * 0.30, h / 2 + 0.03, -d * 0.62), // 멜빵
      B.box(0.055, h * 0.92, 0.045, D, w * 0.30, h / 2 + 0.03, -d * 0.62),
      B.box(w * 0.30, 0.030, 0.035, glow, 0, h * 0.98, d * 0.42)   // 표시등
    ];
    for (let i = 0; i < level; i++) {
      parts.push(B.box(0.032, 0.032, 0.022, glow, -w * 0.26 + i * 0.048, h * 0.14, d * 0.56));
    }
    return parts;
  },
...
/* 낙하산: 램에어(사각) 캐노피 + 산줄
   기하의 원점이 '멜빵이 어깨에 걸리는 자리' 입니다. 예전에는 원점이 캐노피
   쪽이라 줄이 머리 위 허공에서 끊겨 보였습니다. 원점을 어깨에 두면 좌우로
   기울일 때도 사람을 축으로 캐노피가 도는 자연스러운 움직임이 됩니다. */
const ChuteArt = {
  geo: null,
  build() {
    ...
    const B = Build;
    const CELLS = 7;
    const span = 5.4, chord = 2.4, thick = 0.40;
    const H = 3.6;                      // 어깨에서 캐노피까지
    const arc = 0.34;                   // 날개 끝이 아래로 처지는 정도
    const top = 0xe6edf4, under = 0xbcc6d1, rib = 0x39424d;
    const edge = 0x64e2d8, cord = 0x2c333c;
    const parts = [];
    const cw = span / CELLS;
    const anchors = [];                 // 줄을 매달 자리

    for (let i = 0; i < CELLS; i++) {
      const t = (i - (CELLS - 1) / 2) / ((CELLS - 1) / 2);   // -1 … 1
      const ang = t * arc;
      const x = Math.sin(ang) * (span * 0.5);
      const y = H - (1 - Math.cos(ang)) * (span * 0.5);
      parts.push(B.box(cw * 0.92, thick, chord, top, x, y, 0, 0, 0, -ang));
      // 아랫면을 조금 어둡게 해서 두께가 보이게 합니다
      parts.push(B.box(cw * 0.92, thick * 0.22, chord * 0.96, under,
                       x + Math.sin(-ang) * thick * 0.42, y - Math.cos(ang) * thick * 0.42, 0, 0, 0, -ang));
      // 앞전: 위쪽은 청록 띠, 아래쪽은 바람이 들어오는 셀 입구(어둡게)
      parts.push(B.box(cw * 0.92, thick * 0.34, 0.16, edge,
                       x + Math.sin(-ang) * thick * 0.22, y + Math.cos(ang) * thick * 0.22,
                       chord * 0.5, 0, 0, -ang));
      parts.push(B.box(cw * 0.74, thick * 0.40, 0.13, 0x1b2027,
                       x + Math.sin(-ang) * -thick * 0.16, y - Math.cos(ang) * thick * 0.16,
                       chord * 0.5 + 0.02, 0, 0, -ang));
      // 칸막이
      if (i < CELLS - 1) {
        const a2 = (t + 1 / ((CELLS - 1) / 2)) * arc;
        parts.push(B.box(0.05, thick * 1.06, chord * 0.98, rib,
                         (x + Math.sin(a2) * (span * 0.5)) / 2,
                         (y + H - (1 - Math.cos(a2)) * (span * 0.5)) / 2, 0, 0, 0, -(ang + a2) / 2));
      }
      if (i === 0 || i === (CELLS - 1) / 2 || i === CELLS - 1) {
        for (const cz of [chord * 0.30, -chord * 0.30]) {
          anchors.push([x, y - Math.cos(ang) * thick * 0.5, cz]);
        }
      }
    }

    /* 산줄: 어깨(원점)에서 캐노피 아래로.
       상자의 국소 +Y 가 줄 방향을 향하도록 각을 풉니다 (Euler XYZ 기준). */
    for (const [ax, ay, az] of anchors) {
      ...
      parts.push(B.box(0.035, L, 0.035, cord, ax / 2, ay / 2, az / 2, rx, 0, rz));
    }
    // 어깨 멜빵
    parts.push(B.box(0.30, 0.10, 0.16, cord, 0, 0.03, 0));
    parts.push(B.box(0.09, 0.16, 0.10, rib, 0, 0.12, 0));

    this.geo = B.merge(parts);
    return this.geo;
  }
};
...
class Char3D {
  ...
  refreshGear() {
    ...
    if (this.helmet) {
      /* 바닥에 떨어진 헬멧(LootArt.helmetParts)과 같은 생김새를 머리 크기로 줄인 것입니다.
         한쪽만 고치면 주울 때와 썼을 때가 달라 보이므로 함께 손봐야 합니다. */
      const c = HELMETS[this.helmet].color, strap = 0x24282e, M = 0x555d66;
      const glow = this.helmet >= 3 ? 0x64e2d8 : 0x8fa3b5;
      const parts = [
        B.sphere(0.132, c, 0, 0.118, -0.010, 1.0, 0.66, 1.02, 16),      // 낮게 눌린 껍데기
        B.box(0.172, 0.036, 0.082, c, 0, 0.116, 0.112),                 // 앞으로 뻗은 챙
        B.box(0.210, 0.033, 0.086, c, 0, 0.104, -0.102),                // 뒤 목가리개
        B.box(0.044, 0.084, 0.076, strap, -0.114, 0.090, -0.006),       // 귀덮개
        B.box(0.044, 0.084, 0.076, strap, 0.114, 0.090, -0.006),
        B.box(0.142, 0.040, 0.032, glow, 0, 0.084, 0.132),              // 앞 바이저
        B.box(0.150, 0.016, 0.036, strap, 0, 0.108, 0.134),
        B.box(0.028, 0.072, 0.026, strap, -0.104, 0.028, 0.028),        // 턱끈
        B.box(0.028, 0.072, 0.026, strap, 0.104, 0.028, 0.028)
      ];
      if (this.helmet >= 2) {
        parts.push(B.box(0.016, 0.022, 0.102, M, -0.126, 0.136, 0.000));  // 옆 레일
        parts.push(B.box(0.016, 0.022, 0.102, M, 0.126, 0.136, 0.000));
      }
      if (this.helmet >= 3) {
        parts.push(B.box(0.110, 0.022, 0.028, M, 0, 0.196, -0.014));      // 윗면 레일
        parts.push(B.box(0.046, 0.046, 0.046, 0x1f2227, 0, 0.158, 0.122));// 앞쪽 야시경 거치대
      }
    ...
    if (this.bag) {
      ...
      /* 등에 붙는 것이라 뒤로 많이 튀어나오면 총·몸과 한 덩어리로 보입니다.
         납작하게 만들고, 윗덮개를 밝게 해서 실루엣이 나뉘도록 했습니다. */
      const w = 0.23 + this.bag * 0.022, h = 0.22 + this.bag * 0.030, dz = 0.075 + this.bag * 0.013;
      const y0 = -0.19, z0 = -0.125;            // 척추뼈에서 등 한가운데에 붙입니다
      const D = 0x24282e, M = 0x8a939e;
      const glow = this.bag >= 3 ? 0x64e2d8 : 0x8fa3b5;
      const parts = [
        B.box(w, h, dz, c, 0, y0, z0 - dz / 2),                                // 몸통
        B.box(w * 0.96, h * 0.22, dz * 1.10, M, 0, y0 + h * 0.39, z0 - dz / 2),// 윗덮개 (밝게)
        B.box(w * 0.58, h * 0.26, dz * 0.42, D, 0, y0 - h * 0.12, z0 - dz * 1.06), // 뒷주머니
        B.box(w * 0.62, 0.018, 0.022, M, 0, y0 - h * 0.02, z0 - dz * 1.24),    // 지퍼
        B.box(w * 0.26, 0.022, 0.026, glow, 0, y0 + h * 0.47, z0 - dz * 1.02)  // 표시등
      ];
      for (let i = 0; i < this.bag; i++) {
        parts.push(B.box(0.022, 0.022, 0.016, glow, -w * 0.22 + i * 0.036, y0 - h * 0.38, z0 - dz * 1.2));
      }
  ...
  syncMesh(dt, aiming) {
    ...
    if (this.dead) {                                  // 쓰러짐: 앞으로 넘어갑니다
      ...
      if (this.flying === 'chute' && !this.chuteMesh) {
        ...
        this.chuteMesh.position.y = 1.45;   // 어깨 높이 (기하 원점이 멜빵 자리입니다)
