...
const Game = {
  ...
  init(canvas) {
    ...
    this.scene.fog = new THREE.Fog(srgb(THEME.fog), CFG.FOG_NEAR, CFG.FOG_FAR);
    ...
    this.sun = new THREE.DirectionalLight(THEME.starLight, THEME.starPower);
  ...
  buildSky() {
    ...
    for (let i = 0; i < N; i++) {
      // 위쪽에 더 많이 보이도록 y 를 살짝 올려 잡습니다
      const u = Math.random() * 2 - 1, a = Math.random() * Math.PI * 2;
      const y = Math.abs(u) * 0.9 + 0.04, r = Math.sqrt(Math.max(0, 1 - y * y));
      const R = 820;
      pos[i * 3] = Math.cos(a) * r * R;
      pos[i * 3 + 1] = y * R;
      pos[i * 3 + 2] = Math.sin(a) * r * R;
      // 흰색에서 푸른빛·붉은빛까지 살짝 섞습니다
      const t = Math.random();
      c0.setHSL(t < 0.75 ? 0.58 : 0.06, t < 0.75 ? 0.25 : 0.45, 0.72 + Math.random() * 0.28);
    ...
    grad.addColorStop(0.14, 'rgba(214,232,255,0.95)');
    grad.addColorStop(0.38, 'rgba(150,190,255,0.30)');
    grad.addColorStop(1, 'rgba(110,150,255,0)');
    g2.fillStyle = grad; g2.fillRect(0, 0, 128, 128);
    this.sunSprite = new THREE.Sprite(new THREE.SpriteMaterial({
      map: new THREE.CanvasTexture(c), transparent: true,
      depthWrite: false, depthTest: true, fog: false     // 언덕 뒤로 넘어가면 가려집니다
    }));
    this.sunSprite.scale.setScalar(120);
    this.sunSprite.renderOrder = -2;
    this.scene.add(this.sunSprite);

    // --- 지평선에 걸린 큰 행성: 띠가 있는 가스 행성과 고리 ---
    const pc = document.createElement('canvas');
    pc.width = pc.height = 256;
    const g4 = pc.getContext('2d');
    g4.clearRect(0, 0, 256, 256);
    g4.save();
    g4.beginPath(); g4.arc(128, 128, 96, 0, Math.PI * 2); g4.clip();
    const pg = g4.createLinearGradient(0, 32, 0, 224);
    pg.addColorStop(0, '#c98f6a'); pg.addColorStop(0.35, '#a86b52');
    pg.addColorStop(0.6, '#8d5748'); pg.addColorStop(1, '#5f3a38');
    g4.fillStyle = pg; g4.fillRect(0, 0, 256, 256);
    for (let i = 0; i < 9; i++) {                       // 가로 띠
      const y = 40 + i * 20 + Math.random() * 6;
      g4.fillStyle = 'rgba(255,220,190,' + (0.05 + Math.random() * 0.09).toFixed(2) + ')';
      g4.fillRect(0, y, 256, 4 + Math.random() * 7);
    }
    // 어두운 쪽 그림자
    const sh = g4.createRadialGradient(88, 100, 20, 128, 128, 108);
    sh.addColorStop(0, 'rgba(0,0,0,0)'); sh.addColorStop(1, 'rgba(4,2,12,0.82)');
    g4.fillStyle = sh; g4.fillRect(0, 0, 256, 256);
    g4.restore();
    // 고리
    g4.save();
    g4.translate(128, 128); g4.rotate(-0.32); g4.scale(1, 0.22);
    g4.strokeStyle = 'rgba(220,196,170,0.42)'; g4.lineWidth = 13;
    g4.beginPath(); g4.arc(0, 0, 132, 0, Math.PI * 2); g4.stroke();
    g4.strokeStyle = 'rgba(200,176,150,0.24)'; g4.lineWidth = 6;
    g4.beginPath(); g4.arc(0, 0, 152, 0, Math.PI * 2); g4.stroke();
    g4.restore();
    this.planet = new THREE.Sprite(new THREE.SpriteMaterial({
      map: new THREE.CanvasTexture(pc), transparent: true,
      depthWrite: false, depthTest: true, fog: false     // 지형에 가려지도록
    }));
    this.planet.scale.setScalar(330);
    this.planet.renderOrder = -2;
    this.planetDir = new THREE.Vector3(-0.72, 0.13, -0.68).normalize();
    this.scene.add(this.planet);

    // --- 성운: 색이 섞인 옅은 띠가 아주 천천히 흐릅니다 ---
    ...
    for (let i = 0; i < 70; i++) {
      ...
      const gr = g3.createRadialGradient(x, y, 2, x, y, r);
      gr.addColorStop(0, Math.random() < 0.55 ? THEME.nebula1 : THEME.nebula2);
      gr.addColorStop(1, 'rgba(0,0,0,0)');
      g3.fillStyle = gr;
      g3.beginPath(); g3.arc(x, y, r, 0, Math.PI * 2); g3.fill();
    }
    const cloudTex = new THREE.CanvasTexture(cc);
    cloudTex.wrapS = cloudTex.wrapT = THREE.RepeatWrapping;
    cloudTex.repeat.set(3, 2);
    /* 판이 아니라 하늘 돔 안쪽에 입힙니다.
       판으로 깔면 하늘 한가운데에 판의 모서리가 직선으로 드러납니다. */
    const cloudGeo = new THREE.SphereGeometry(860, 32, 20);
    this.clouds = new THREE.Mesh(cloudGeo, new THREE.MeshBasicMaterial({
      ...
      fog: false, side: THREE.BackSide, blending: THREE.AdditiveBlending
    }));
  ...
  start(botCount, opts) {
    ...
    for (const g of (this.nades || [])) this.scene.remove(g.mesh);
    for (const g of (this.smokes || [])) this.scene.remove(g.mesh);
    ...
    for (let i = 0; i < botCount; i++) {
      ...
      if (Math.random() < 0.45) b.throws.frag = 1 + Math.floor(Math.random() * 2);
      if (Math.random() < 0.30) b.throws.smoke = 1;
  ...
  spawnLoot() {
    const add = (x, z, y) => {
      ...
      if (roll < 0.34) {
        ...
        const t = rnd() < 0.6 ? 'frag' : 'smoke';
        l = new Loot(x, z, 'throw', t, 1 + Math.floor(rnd() * 2), 0, y);
      } else {
  ...
  update(dt, input) {
    ...
    // 하늘에 있는 것들은 카메라를 따라다닙니다 (멀리 있는 것처럼 보이게)
    const cp = this.camera.position;
    this.sunSprite.position.set(cp.x + sd.x * 800, cp.y + sd.y * 800, cp.z + sd.z * 800);
    this.stars.position.set(cp.x, cp.y, cp.z);
    const pd = this.planetDir;
    this.planet.position.set(cp.x + pd.x * 760, cp.y + pd.y * 760, cp.z + pd.z * 760);
    this.clouds.position.set(cp.x, cp.y, cp.z);
    this.clouds.material.map.offset.x = (this.time * 0.0004) % 1;   // 성운은 아주 천천히
  ...
  dropLoot(c) {
    ...
    for (const t of THROW_KEYS) {
      if ((c.throws[t] || 0) <= 0) continue;
      const l = new Loot(c.pos.x + (t === 'frag' ? 1.5 : -1.5), c.pos.z + 1.2, 'throw', t,
                         c.throws[t], 0, c.pos.y);
      this.scene.add(l.mesh); this.loots.push(l);
    }
    if (c.helmet) {
  ...
  pickUp(ch, l) {
    ...
    } else if (l.kind === 'throw') {
      const have = ch.throws[l.gun] || 0, cap = ch.throwCap;
      if (have >= cap) {
        if (ch === this.player) this.pushFeed(THROWABLES[l.gun].name + ' 을 더 들 수 없습니다 (가방을 구하세요)');
        return false;
      }
      const got = Math.min(l.amount, cap - have);
      ch.throws[l.gun] = have + got;
      l.amount -= got;
      if (ch === this.player) this.pushFeed(THROWABLES[l.gun].name + ' +' + got + '개');
      if (l.amount > 0) {                                // 남으면 그대로 바닥에 둡니다
  ...
  /* 조준한 방향으로 던집니다 */
  throwItem(ch, type) {
    if (ch.dead || ch.flying || ch.climb) return false;
    if ((ch.throws[type] || 0) <= 0) {
      if (ch === this.player) this.pushFeed(THROWABLES[type].name + ' 이(가) 없습니다');
      return false;
    }
    ch.throws[type]--;
    let dx, dy, dz;
    if (ch === this.player) {
      ...
      const t = ch.ai && ch.ai.target;
      const tx = t ? t.pos.x : ch.pos.x + Math.sin(ch.yaw) * 20;
      const tz = t ? t.pos.z : ch.pos.z + Math.cos(ch.yaw) * 20;
      const l = Math.hypot(tx - ch.pos.x, tz - ch.pos.z) || 1;
      dx = (tx - ch.pos.x) / l; dz = (tz - ch.pos.z) / l; dy = 0;
    }
    // 조금 위로 들어 올려 포물선을 그리게 합니다
    const up = Math.min(0.85, Math.max(-0.4, dy + CFG.THROW_UP));
    const flat = Math.sqrt(Math.max(0.02, 1 - up * up));
    const hl = Math.hypot(dx, dz) || 1;
    const sp = CFG.THROW_SPEED;
    const ox = ch.pos.x + dx / hl * 0.5, oz = ch.pos.z + dz / hl * 0.5;
    const oy = ch.pos.y + (ch.crouch ? 0.85 : 1.05);
    const g = new Grenade(type, ox, oy, oz,
                          dx / hl * flat * sp, up * sp, dz / hl * flat * sp, ch);
    this.scene.add(g.mesh);
    this.nades.push(g);
    Sfx.swap();
    if (ch === this.player) this.pushFeed(THROWABLES[type].name + ' 투척 (남은 ' + ch.throws[type] + '개)');
    return true;
  },
  ...
  updateThrown(dt) {
    for (let i = this.nades.length - 1; i >= 0; i--) {
      ...
      if (g.fuse <= 0) {
        ...
        if (g.type === 'frag') this.explode(g);
        else this.makeSmoke(g.pos.x, g.pos.y, g.pos.z);
      }
    }
    for (let i = this.smokes.length - 1; i >= 0; i--) {
      const sm = this.smokes[i];
      sm.update(dt);
      if (sm.dead) { this.scene.remove(sm.mesh); this.smokes.splice(i, 1); }
    }
  },

  /* 수류탄 폭발: 벽에 가려지지 않은 사람만 거리에 따라 피해를 봅니다 */
  explode(g) {
    const T = g.spec, cx = g.pos.x, cy = g.pos.y + 0.2, cz = g.pos.z;
    for (const c of this.chars) {
      if (c.dead) continue;
      const hx = c.pos.x, hy = c.pos.y + 0.8, hz = c.pos.z;
      const d = Math.hypot(hx - cx, hy - cy, hz - cz);
      if (d > T.radius) continue;
      if (!World.clear(cx, cy, cz, hx, hy, hz)) continue;      // 벽 뒤는 안전합니다
      const k = 1 - d / T.radius;
      const dmg = T.minDmg + (T.dmg - T.minDmg) * k * k;
      this.damage(c, dmg, g.owner, false, false);
    }
    // 차량도 부숩니다
    for (const v of this.vehicles) {
      if (v.dead) continue;
      const d = Math.hypot(v.pos.x - cx, v.pos.z - cz);
      if (d < T.radius) v.hp -= (T.dmg * 1.6) * (1 - d / T.radius);
    }
    // 눈에 보이는 효과
    for (let i = 0; i < 14; i++) {
      this.puff(cx + (Math.random() - 0.5) * 2.6, cy + Math.random() * 2.2,
                cz + (Math.random() - 0.5) * 2.6, i < 5 ? 0xffb057 : 0x6b6257);
    }
    this.muzzleFlash(cx, cy + 0.6, cz, false);
    this.flashLight.intensity = 30;
    this.flashT = 0.18;
    const pd = this.player.pos.distanceTo(g.pos);
    Sfx.blast(pd);
    if (pd < 26) this.landDip = Math.min(0.5, (1 - pd / 26) * 0.5);
  },

  makeSmoke(x, y, z) {
    const gy = World.groundY(x, z, y + 1);
    const sm = new Smoke(x, Math.max(gy, y - 0.2), z);
    this.scene.add(sm.mesh);
    this.smokes.push(sm);
    Sfx.smoke(this.player.pos.distanceTo(sm.pos));
  },

  /* 이 선분이 연막에 가려지는가 */
  smoked(x1, y1, z1, x2, y2, z2) {
    for (const sm of this.smokes) if (sm.blocks(x1, y1, z1, x2, y2, z2)) return true;
    return false;
  },
  ...
  buildMinimapImage() {
    ...
    for (let j = 0; j < S; j++) {
      for (let i = 0; i < S; i++) {
        ...
        const k = (j * S + i) * 4;
        let r, gg, b;
        // 지형 색과 같은 계열로 (초록 지도가 붉은 땅 위에 뜨면 따로 놉니다)
        if (h < World.waterY) { r = 18; gg = 63; b = 74; }
        else if (h < World.waterY + 1.6) { r = 139; gg = 116; b = 87; }
        else {
          const t = Math.max(0, Math.min(1, (h - 2) / 26));
          r = 92 + t * 22; gg = 62 + t * 24; b = 44 + t * 46;
