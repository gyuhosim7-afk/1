...
const CFG = {
  ...
  DUEL_WINS: 7,           // 먼저 이만큼 점수를 내면 승리
  DUEL_ROUND: 2.0,        // 한쪽이 쓰러진 뒤 다음 판이 시작되기까지 (초)
  DUEL_SAFE: 1.2,         // 새 판 시작 직후 무적 시간 (스폰 지점 사냥 방지)
  FOG_NEAR: 70,
  FOG_FAR: 390,
  FOV: 72,
  ADS_FOV: 42,
  ...
  SPRINT: 7.8,
  CROUCH: 2.2,
  /* 봇 이동 속도 배수. 봇은 사람과 같은 속도 규칙(앞으로만 질주, 정조준 중
     감속 등)을 지키므로 1 이면 사람과 완전히 같은 속도입니다. 어느 쪽도
     유리하지 않게 1 로 둡니다. 난이도를 낮추고 싶을 때만 내리세요. */
  BOT_SPEED: 1.0,
  ...
  /* 봇이 볼 수 있는 좌우 각도(가로, 도). 사람은 화면에 담긴 만큼만 보므로
     — 세로 72° 를 16:9 화면으로 펴면 가로 약 105° — 봇도 같게 둡니다.
     이 각도 밖은 아래 BOT_HEAR 안쪽에서만 알아챕니다(발소리·총성). */
  BOT_FOV: 105,
  BOT_HEAR: 20,           // 등 뒤라도 이 거리 안이면 알아챕니다 (m)
  /* 총성이 들리는 거리(m). 결투장은 밀폐된 88×64m 공간이라 어디서 쏘든
     들립니다 — 사람도 위치 기반 소리로 봇의 총성을 맵 전체에서 듣습니다.
     한쪽만 못 듣게 두면 그것도 불공평합니다. */
  BOT_GUNSHOT: 110,

  /* ---------- 봇 조준 ----------
     사람과 같은 조건으로 맞춘 값들입니다. 둘로 나뉩니다.

     남겨 두는 것 — 사람의 눈과 손을 흉내내는 값입니다. 즉시 180° 돌아보는
     것은 '공평' 이 아니라 사람이 할 수 없는 일이므로, 사람이 실제로 내는
     수치에 맞춰 둡니다.
     없앤 것 — 사람에게는 없는데 봇에게만 있던 인공 오차입니다. */
  BOT_TURN: 7.5,          // 조준 회전 속도 (rad/s) — 90° 를 0.21초에 돌리는 정도
  BOT_REACT: 0.22,        // 적을 보고 반응하기까지 (초) — 사람 평균 반응 속도
  /* 조준점이 목표 중심에서 벗어나는 정도(라디안). '사람의 조준점 놓기'
     오차입니다 — 0 으로 두면 봇의 조준점이 매 발 정확히 목 한가운데에
     놓여, 사람이 낼 수 없는 정확도가 됩니다(15m 에서 명중 90% · 머리 72%).
     0.03 은 실력 있는 사람 정도입니다: 15m 에서 명중 약 87% · 머리 약 34%,
     35m 로 멀어지면 명중 20%대로 떨어집니다. */
  BOT_AIM_ERR: 0.03,
  /* 겨누는 높이(m). 머리 판정은 1.44m 위입니다.
     가슴(1.0)만 노리면 머리를 노리는 사람이 일방적으로 유리하고,
     머리(1.62)만 노리면 사람보다 정확해집니다. 목 부근을 겨눠
     탄퍼짐에 따라 머리와 몸이 섞이게 둡니다. */
  BOT_AIM_Y: 1.35,
...
/* ============================================================
   화면 분위기 (색을 한 곳에 모아 둡니다)
   외계 행성의 밤: 검보라 하늘에 별과 성운, 붉은 흙과 보랏빛 암반.
   ============================================================ */
const THEME = {
  skyTop:    0x090a1a,        // 하늘 꼭대기 (거의 검은 남색)
  skyBottom: 0x3b2440,        // 지평선 (먼지 낀 보라)
  fog:       0x2b1f33,
  background:0x0c0a1a,

  starLight: 0xdbe6ff,        // 멀리 있는 항성 — 희고 차갑습니다
  starPower: 1.45,
  hemiSky:   0x5a4a86, hemiGround: 0x2e1c14, hemiPower: 0.60,
  ambient:   0x6d5f80, ambientPower: 0.22,
  fillColor: 0x7f6fb8, fillPower: 0.5,

  ground1:   0x4a3021,        // 붉은 흙 (밤이라 어둡게)
  ground2:   0x5c3e2a,
  rock:      0x3a3444,        // 보랏빛 암반
  sand:      0x6a5942,        // 물가
  road:      0x433c4c,
  water:     0x123f4a,        // 검은 청록

  trunk:     0x3b3039,        // 외계 식생
  pine:      [0x27404c, 0x2f4b58, 0x395a66],
  leaf:      [0x3a2f54, 0x463b62, 0x322947],

  nebula1:   'rgba(140,90,200,0.30)',   // 성운 두 겹
  nebula2:   'rgba(70,150,190,0.22)',

  /* 건물 색. 지구식 흙빛 배색은 그대로 두고, 그릴 때 이 띠 안으로 옮깁니다.
     벽·지붕·창틀의 밝기 차이는 남으므로 지금처럼 구분되어 보입니다. */
  buildHue:    0.605,          // 기준 색상 (청보라)
  buildSpread: 0.17,           // 원래 색상에 따라 이만큼까지 벌어집니다
  buildSat:    0.30,           // 채도 상한 — 금속·콘크리트처럼 차분하게
  buildLit:    0.52            // 밝기 배율 (밤이라 낮게)
};
